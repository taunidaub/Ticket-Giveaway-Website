				...
				if($table==$this->config->item('mu')){
					if($gid!=''){
						...
						foreach ($query->result() as $row)
						{
							...
							<form id="myForm" action="<?php echo $this->config->item('base_url') ?>main/confirm/<? echo ($gid."/") ?>">
							<?php
							...
							?>					
								<div class="label" style="width:45%"><strong>Name: </strong><?php echo $row2->entry_name ?></div><div class="label"><input type="radio" name="confirm[<?php echo $r ?>]" value='declined:<?php echo $row2->entry_id ?>' /> Declines &nbsp;<input type="radio" name="confirm[<?php echo $r ?>]" value='confirmed:<?php echo $row2->entry_id ?>' /> Accepts</div><br>
							<?
								$r++;
							}//	foreach ($query2->result() as $row2){	
							if($r>0){
							?><br><br>
							<input name="submit" id="submit" type="submit" value="Submit Selections" onClick="javascript:confirm_winners(<? echo ($gid) ?>);" />					
							<? } ?>		
							</form>
							</div>
